...
def grafo():
    ...
    afe = Digraph('L={anbkc(n+k)∣n≥1k≥1', filename='aut_pilha_ex2', format='png')
    ...
    afe.edge('q0', 'q0', label='b, ϵ, B')
    afe.edge('q0', 'q1', label='c, B, ϵ')
    afe.edge('q1', 'q1', label='c, [A, B], ϵ')
...
def test_pda(pda, palavra):
    ...
    count_c = 0 # conta número de c's da palavra
    for char in palavra:
    ...
    if teste == False: # Verificação de passo
        result = False
    if count_a == 0 or count_b == 0: # Verificação de palavra
        result = False
    if (count_a + count_b) != count_c: # Verificação de palavra
        result = False
