...
Instruções adicionais:
    ...
    Passo 1 -
        ...
        Leia os valores dos neurônios de entrada como double
    ...
    Passo 3 - Calcule a saída final
        A saída final da rede é calculada a partir dos outputs da camada oculta e COM 2 CASAS DECIMAIS de precisão.
...
void multilayer_perceptron() {
    ...
    float *entrada;  // Vetor de entrada (features) com 2 elementos
    float pesos_oculta[2][2] = {{0.5, 0.5}, {0.4, 0.6}};  // Matriz de pesos da camada oculta (2 neurônios x 2 entradas)
    float pesos_saida[2] = {0.7, 0.3};  // Vetor de pesos da camada de saída (1 neurônio x 2 entradas da camada oculta)
    float saida_oculta[2];  // Saída da camada oculta
    float saida_final;  // Saída final da rede
    int i, j;

    entrada = (float *) malloc(2 * sizeof(float));

    // Leitura do vetor de entrada
    //printf("Digite os valores de entrada (2 valores):\n");
    for (i = 0; i < 2; i++) {
        scanf("%f", &entrada[i]);
    }

    // Cálculo da saída da camada oculta
    for (i = 0; i < 2; i++) {  // Para cada neurônio na camada oculta
        saida_oculta[i] = 0;
        for (j = 0; j < 2; j++) {  // Para cada entrada
            saida_oculta[i] += entrada[j] * pesos_oculta[i][j];
        }
        saida_oculta[i] = sigmoide(saida_oculta[i]);
    }

    // Cálculo da saída final
    saida_final = 0;
    for (i = 0; i < 2; i++) {
        saida_final += saida_oculta[i] * pesos_saida[i];
    }
    saida_final = sigmoide(saida_final);
