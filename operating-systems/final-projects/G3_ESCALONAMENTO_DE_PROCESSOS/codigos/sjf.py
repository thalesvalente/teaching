...
def main():
    ...
    #ordena processos de acordo com o tempo de computação, 
    #especificando a chave de ordenação como o segundo elemento de cada lista (que é nosso tempo de comp com indice 1)
    processos = sorted(processos, key=lambda x: x[1])
    ...
    #loop para calcular o turnaround
    total_turnaround = 0
    for i in range(prc):
    ...
    ax.set_title('Gantt Chart - Shortest Job First')
    plot.ylim((0,5))
    ax.set_yticklabels([])
    ax.set_xticklabels([])
